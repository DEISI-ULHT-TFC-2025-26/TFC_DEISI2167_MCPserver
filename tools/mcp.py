...
def register_mcp_tools(mcp: FastMCP, db: DB):
    ...
    @mcp.tool(description="Executes a SELECT query and exports the results to a JSON file. Returns the URL to download the file.")
    def export_query_to_json(query: str, filename: str) -> str:
        global dw
        return dw.export_query_to_json(query, filename)


    @mcp.tool(description="Executes a SELECT query and exports the results to an Excel (.xlsx) file. Returns the URL to download the file.")
    def export_query_to_excel(query: str, filename: str) -> str:
        global dw
        return dw.export_query_to_excel(query, filename)